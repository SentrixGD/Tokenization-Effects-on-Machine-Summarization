...
def main():
    ...
    train_processed = pd.read_csv(os.path.join(processed_file_dir, "BPE_augmented.csv"))
    train_corpus = "\n".join(
    ...
    tokenizer.train(
        train_corpus,
        output_dir=os.path.join(ROOT_DIR, "tokenizers", "BPE"),
        model_prefix=model_prefix,
        vocab_size=vocab_size,
    )
    tokenizer.load(os.path.join(ROOT_DIR, "tokenizers", "BPE", f"{model_prefix}.model"))
