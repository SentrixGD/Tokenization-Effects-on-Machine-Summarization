"""
Script: tokenizer_training.py
Purpose: Train a tokenizer on a text corpus, compute tokenization statistics, and save the tokenizer model, vocabulary and filtered dataset for downstream NLP tasks.
...
def encode_stats(
    ...
    """
    Encode a string using a trained tokenizer and compute encoding statistics.
...
def main(tokenizer_name: str, vocab_size: int):
    # ------------------------------------------------------------
    # Loading the data
    # ------------------------------------------------------------
    ...
    model_prefix = f"{tokenizer_name}_tokenizer"
    tokenizer.train(
        train_corpus,
        output_dir=os.path.join(ROOT_DIR, "tokenizer", f"{tokenizer_name}"),
        model_prefix=model_prefix,
        vocab_size=vocab_size,
    )
    tokenizer.load(
        os.path.join(
            ROOT_DIR, "tokenizer", f"{tokenizer_name}", f"{model_prefix}.model"
        )
    )
    ...
    plt.savefig(
        os.path.join(ROOT_DIR, "data", "stats", f"{tokenizer_name}_article_length.png")
    )
    plt.close()
    ...
    plt.ylabel("Density")
    plt.title(f"{tokenizer_name} Compression Ratio Distribution")
    plt.savefig(
        os.path.join(
            ROOT_DIR, "data", "stats", f"{tokenizer_name}_compression_distribution.png"
        )
    )
    plt.close()
    ...
    stats_path = os.path.join(ROOT_DIR, "data", "stats", f"{tokenizer_name}_stats.json")
    tokenizer_stats = {
        "meta": {
    ...
    with open(stats_path, "w") as f:
        json.dump(tokenizer_stats, f, indent=4)

    # save the tokenized data
    train_filtered[["cleaned_text", "highlights"]].to_csv(
        os.path.join(ROOT_DIR, "data", "processed", f"{tokenizer_name}_train.csv"),
        index=False,
    )
...
if __name__ == "__main__":
    ...
    main(tokenizer_name=args.tokenizer, vocab_size=args.vocab_size)
